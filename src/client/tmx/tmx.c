...
void tmx_map_free(tmx_map *map)
{
	if (map)
	{
		...
		if (map->format_version)
			tmx_free_func(map->format_version);
		if (map->class_type)
			tmx_free_func(map->class_type);
		tmx_free_func(map);
	}
}
...
tmx_tileset_list *tmx_find_tileset_by_name(const tmx_map *map, const char *name)
{
	...
	{
		if (res->tileset && !strcmp(res->tileset->name, name))
			return res;
	}
	return NULL;
}
